...
class Loader_MiBand6:
	...
	def render(self):
		w = 152
		h = 486
		pvd = PreviewDrawer.new(size=(w, h))
		pvd.putImages(self.images)
		...
		if "Date" in config:
			if "MonthAndDayAndYear" in config["Date"]:
				...
				if "OneLine" in config["Date"]["MonthAndDayAndYear"]:
					dotIndex = config["Date"]["MonthAndDayAndYear"]["OneLine"]["DelimiterImageIndex"]
		...
		o = pvd.getCanvas()
		m = Image.open(ROOT_DIR+"/res/mb6_mask.png").convert("L")
		o.putalpha(m)
		return o
